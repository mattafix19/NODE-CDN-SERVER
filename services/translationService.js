...
var ip2long = ipUtils.ip2long;
var cidr = require('cidr.rb');
var translateFunctions = require('../helpers/translateFunctions');
        ...
        .then(function (foundRfqdn) {
            ...
            redisService.listRangeAsync("remoteEndpointLocalIds", 0, -1)
                .then(function (foundRemoteInterfaces) {
                    ...
                    var footprintsCallbacks = 0;
                    //loop through all found remote interfaces
                    translateFunctions.functionFindFootprints(foundRemoteInterfaces)
                        .then(function (foundFootprint) {
                            console.log('all done');
                            for (var i = 0; i < foundFootprint.length; i++) {
                                for (var j = 0; j < foundFootprint[i].length; j++) {

                                    var deserializedFootprint = JSON.parse(foundFootprint[i][j]);
                            ...
                            // check if were all interfaces done if so, continue
                            var higherPrefix = 0;
                            translateFunctions.functionFindOrigins(foundFootprintsMatches)
                                .then(function (foundOrigins) {

                                    //since redis is single threaded we can use indexes of foundOrigins instead of loop
                                    // through foundFootprintsMatches, so we loop through foundOrigins and we use its indexes in foundFootprintsMatches
                                    for (var k = 0; k < foundFootprintsMatches.length; k++) {
                                        for (var l = 0; l < foundOrigins[k].length; l++) {

                                            var parsedRemoteEndpoint = JSON.parse(foundOrigins[k][l]);
                                            ...
                                            if (name === originName && originFqdn === originServer) {
                                                foundFootprintsMatches[k].fqdn = rfqdn;
                                                if (foundFootprintsMatches[k].prefix > higherPrefix) {
                                                    higherPrefix = foundFootprintsMatches[k].prefix;
                                                    finalFqdn = foundFootprintsMatches[k].fqdn;
                                                }
                                            }
                                        }
                                    }
                                    if (finalFqdn != '' || finalFqdn != undefined || finalFqdn != null) {
                                        if (content != "/") {
                                            var finalUrl = protocol + "://" + finalFqdn + content;
                                        }
                                        else {
